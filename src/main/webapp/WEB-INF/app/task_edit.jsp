...
--%>
                ...
                <label for="task_pstatus">Preis:</label>
                <div class="side-by-side margin">
                    <select name="task_pstatus">
                        <c:forEach items="${pstatuses}" var="pstatus">
                               <option value="${pstatus}" ${task_form.values["task_pstatus"][0] == pstatus ? 'selected' : ''}>
                                <c:out value="${pstatus.label}"/>
                            </option>
                        </c:forEach>
                    </select>
                    <input type="preis" name="task_preis" value="${task_form.values["task_preis"][0]}">
                </div>
                ...
                <div class="side-by-side">
                 ...
                 <label for="task_due_date">Angelegt am:</label>
                <div class="side-by-side">
                   
                    <p name="task_due_date">${task_form.values["task_due_date"][0]} ${task_form.values["task_due_time"][0]}
                    </p>
                </div>
                    
                                       
                
                    <label for="task_anbieter">Anbieter:</label>
                    <div class="side-by-side">
                    <p name="task_anbieter" readonly="readonly">
                       ${task_form.values["user_name"][0]} 
                       </br> ${task_form.values["user_strasse"][0]}  
                       </br> ${task_form.values["user_plz"][0]} ${task_form.values["user_ort"][0]} 
                       </br> ${task_form.values["user_telefon"][0]} 
                    </p>
                </div>
